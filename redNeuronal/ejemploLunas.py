...
puntos en forma de medias lunas. Como se puede ver, la red tiene 3 capas de 2,
6 y 1 neuronas. La segunda capa tiene como función de activación la función
ReLU, mientras que la última tiene la función sigmoide, ya que necesitamos que
el resultado de las predicciones esté entre 0 y 1. La primera capa simplemente
almacena los valores de entrada, así que no tiene función de activación.
...
nFotogramas = 75
nIteraciones = 20 # Aumentar si se ve muy lento

# Creamos las dos medias lunas con la función make_moons
X, Y = make_moons(n_samples = nObservaciones, noise = 0.1)
Y = Y[:, np.newaxis]
...
# Creamos la red
rn = RedNeuronal()
rn.adjuntarCapa(2, 6, 'ReLU')
rn.adjuntarCapa(6, 1, 'sigmoide')

# Preparamos la cuadrícula sobre la que iremos mostrando las predicciones
_x = np.linspace(-1.5, 2.5, nPuntosLadoCuadricula)
_y = np.linspace(-1, 1.5, nPuntosLadoCuadricula)
costes = []
